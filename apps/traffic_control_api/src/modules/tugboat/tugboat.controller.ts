...
export class TugboatController {
  ...
  async updateTugboatById(@Param() param: { tugboatId: number }, @Body() updateTugboat: CreateTugboatDTO): Promise<UpdateResult> {
    return await this.tugboatService.updateTugboatById(param.tugboatId, updateTugboat);
  }

  @Delete(':tugboatId/delete')
  async deleteTugboat(@Param() param: { tugboatId: number }): Promise<DeleteResult> {
